...
describe 'ravendb::install' do
  context 'with default values for all parameters' do
    ...
    it {
      is_expected.to contain_file('C:\\RavenDB-3.5.4.Setup.exe').with(ensure: 'file',
                                                                      source: 'https://daily-builds.s3.amazonaws.com/RavenDB-3.5.4.Setup.exe')
    }

    it {
      is_expected.to contain_package('RavenDB').with(ensure: 'present',
    ...
    it {
      is_expected.to contain_file('C:\\RavenDB-3.5.4.Tools.zip').with(ensure: 'absent',
  ...
  context 'with package_ensure => absent' do
    ...
    it {
      is_expected.to contain_file('C:\\RavenDB-3.5.4.Setup.exe').with(ensure: 'absent',
                                                                      source: 'https://daily-builds.s3.amazonaws.com/RavenDB-3.5.4.Setup.exe')
    }

    it {
      is_expected.to contain_package('RavenDB').with(ensure: 'absent').that_requires('File[C:\\RavenDB-3.5.4.Setup.exe]')
    }
  end
  ...
  context 'with include_management_tools => true' do
    ...
    it {
      is_expected.to contain_file('C:\\RavenDB-3.5.4.Tools.zip').with(ensure: 'file',
                                                                      source: 'https://daily-builds.s3.amazonaws.com/RavenDB-3.5.4.Tools.zip')
    }

    it {
      is_expected.to contain_dsc_archive('Unzip RavenDB Tools').with(ensure: 'present',
